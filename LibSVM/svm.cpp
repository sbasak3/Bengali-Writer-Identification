...
{
	int i,nj,correct=0,currentindex;
	...
	for(i=0;i<nr_fold;i++)
	{
		   ...
		   (param->svm_type == C_SVC || param->svm_type == NU_SVC))
		{
			...
			for(j=begin;j<end;j++)
			{
				...
				if(target[perm[j]]== prob->y[currentindex])
					correct++;
			     for(nj=0;nj<2;nj++)
				 {  //// write every prob-x[perm[]] to a text where the highest corresponding prob_estmates is less than 80 and also diffrence btwwen the highest and  second higest is less than 25.....file along with the target[prem[j]] the label, this text file will be processed at the end to get over all accuracy and we can have separate test file or five fold on thhis also. 
			     	//printf("The %d  choice Value is %f  and it got classified as %g\n ",nj,prob_estimates[nj],target[perm[j]]);
				///getchar();
				 }
			}
			free(prob_estimates);	
