...
int total_correct_all;
 int *errorprob;
extern struct svm_problem probofshape;
 int *indexerror;

 int passmain(int *errorprob,int l);
 double firstprobabilty(double *);
 int writematrix(int **r);
...
{
	int i,nj,j,correct=0,currentindex,newk,label,ori;
	int *fold_start = Malloc(int,nr_fold+1);
	int l = prob->l;struct svm_node *myvector;
    errorprob=(int*)malloc(l*sizeof(int));
	int **recognitionmatrix;
	int newi;
	int *perm = Malloc(int,l);
	double highestprobability=0;
	int nr_class;
    FILE *newtext=fopen("mytestfile.txt","a+");;
	    ...
	    param->svm_type == NU_SVC) && nr_fold < l)
	{
		...
		for(i=0;i<l;i++)
			index[i]=perm[i];
		recognitionmatrix=(int**)malloc(6*sizeof(int*));
		for(i=0;i<6;i++)
			recognitionmatrix[i]=(int*)malloc(6*sizeof(int));
		for(i=0;i<6;i++)
			for(j=0;j<6;j++)
	...
	for(i=0;i<nr_fold;i++)
	{
		   ...
		   (param->svm_type == C_SVC || param->svm_type == NU_SVC))
		{
			...
			for(j=begin;j<end;j++)
			{
				...
				currentindex=perm[j];
				myvector=prob->x[perm[j]];
				/*fprintf(newtext,"%f ",prob->y[currentindex]);
				while(myvector->index!=-1)
				{
			    fprintf(newtext,"  %d: %f ",myvector->index,myvector->value);
				myvector++;
				}
				fprintf(newtext,"\n");*/
				target[perm[j]] = svm_predict_probability(submodel,prob->x[perm[j]],prob_estimates);
				//
				highestprobability=firstprobabilty(prob_estimates);
				if((target[perm[j]]== prob->y[currentindex])&&(highestprobability>=0.50))
				{
					correct++;
					label=(int)prob->y[currentindex];
					ori=(int)target[perm[j]];
					recognitionmatrix[label][ori]++;
	...
	printf(" Total correct is  %d\n",correct);
    total_correct_all=correct;
    getchar();
	//passmain(errorprob,l);
	//writematrix(recognitionmatrix);
	fclose(newtext);
	free(fold_start);
	free(perm);	
}
...
/*int writematrix(int **r)
{
	FILE *fp;
	int i,j;
	fp=fopen("recognition64-50.txt","a+");
	for(i=0;i<3;i++)
	{   fprintf(fp,"%d ",i);
		for(j=0;j<3;j++)
		{
